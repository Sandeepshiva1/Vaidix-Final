...
import { useDemoState, stepCompletion, PREP_STEPS, type DemoStepKey } from '@/components/demo/demo-state'
import { SessionHeader } from '@/components/demo/session-header'

const ICON_MAP: Record<DemoStepKey, React.ReactNode> = {
...
const CHECKLIST_LABEL: Record<DemoStepKey, string> = {
...
export default function ReadyPage() {
  ...
  const gatingSteps: DemoStepKey[] = ['studio', 'promo', 'analytics', 'questions']
  ...
  const doStart = () => {
    ...
    markStep(session.id, 'ready', true)
    setTimeout(() => router.push(`/demo/sessions/${session.id}/live`), 700)
  }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <SessionHeader session={session} backHref={`/demo/sessions/${session.id}/prepare`} eyebrow="Step 5 · Session Ready" />
