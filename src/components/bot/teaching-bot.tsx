...
// ════════════════════════════════════════════════════════════════════════════
// TeachingBot — the Teaching & Reflection Bot UI.
// ════════════════════════════════════════════════════════════════════════════
// Shared client component rendered by BOTH the demo route (/demo/bot) and the
// real platform route (/bot). Pedagogical flow per question: answer → case
// example → optional quiz → reflective practice. Chat history persists to
// localStorage so it survives refreshes.
//
// Today the responses come from a curated, keyword-matched knowledge base
// (KNOWLEDGE_BASE below). The data source is isolated in `sendMessage` so it can
// be swapped for a live AI call (the configured Gemini router) without touching
// the rest of the UI.
...
interface BotMsg {
  ...
  /** Transient: a bot bubble awaiting the AI reply (renders a typing indicator). */
  pending?: boolean
}
...
// Calls the live AI route. Returns null on any failure so the caller can fall
// back to the curated knowledge base — the bot must never hard-fail.
async function fetchAiReply(question: string): Promise<BotReply | null> {
  try {
    ...
    const body = (await res.json()) as { ok: boolean; data?: BotReply }
...
export function TeachingBot({ heightClass = 'h-[calc(100dvh-9.5rem)]' }: { heightClass?: string }) {
  ...
  useEffect(() => {
    // Never persist a transient "typing" bubble — only settled messages.
    const settled = messages.filter((m) => !m.pending)
    if (settled.length > 1) {
      try { localStorage.setItem(BOT_CHAT_KEY, JSON.stringify(settled)) } catch { /* fine */ }
    }
  ...
  const sendMessage = async () => {
    ...
    const botId = uid()
    // Append the user turn + a pending bot bubble (typing indicator) in one go.
    setMessages((prev) => [...prev, userMsg, { id: botId, role: 'bot', text: '', pending: true }])

    // Live AI first; on any failure fall back to the curated knowledge base so
    // the bot always answers (missing key, offline, rate-limited, bad shape…).
    const ai = await fetchAiReply(text)
    const reply: BotReply = ai ?? (() => {
      const match = matchKnowledge(text)
      const kb = match ?? DEFAULT_RESPONSE
      return {
        answer: match ? match.answer : DEFAULT_RESPONSE.answer,
            ...
            {msg.role === 'user' ? (
              ...
              <div className="flex items-start gap-2.5 max-w-full">
                ...
                <div className="min-w-0 flex-1 space-y-3">
                  {/* Answer text, or a typing indicator while the AI reply is in flight */}
                  {msg.pending ? (
                    <div className="inline-flex items-center gap-1.5 rounded-2xl rounded-tl-sm border border-border/60 bg-card px-4 py-3.5" aria-label="Assistant is typing">
